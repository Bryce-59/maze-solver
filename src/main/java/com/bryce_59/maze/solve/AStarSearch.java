...
public class AStarSearch extends SearchAlgorithm {
    ...
    private class ManhattanComparator implements Comparator<List<Maze.Node>> {
        ...
        public int compare(List<Maze.Node> lhs, List<Maze.Node> rhs) {
            ...
            int lhs_dist = lhs.size()
                    + (Math.abs(getEnd().getX() - lhs_prev.getX()) + Math.abs(getEnd().getY() - lhs_prev.getY()));
            int rhs_dist = rhs.size()
                    + (Math.abs(getEnd().getX() - rhs_prev.getX()) + Math.abs(getEnd().getY() - rhs_prev.getY()));
    ...
    // Maze.Node lhs_prev = lhs.get(lhs.size() - 1);
    // Maze.Node rhs_prev = rhs.get(rhs.size() - 1);
    // int lhs_dist = lhs.size() + (int) (Math.pow(getEnd().getX() -
    // lhs_prev.getX(), 2) + Math.pow(getEnd().getY() - lhs_prev.getY(), 2));
    // int rhs_dist = rhs.size() + (int) (Math.pow(getEnd().getX() -
    // rhs_prev.getX(), 2) + Math.pow(getEnd().getY() - rhs_prev.getY(), 2));
    // return lhs_dist - rhs_dist;
    // }
    // }
}
